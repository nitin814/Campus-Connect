...
export const updatePersonalCode = (socketId) => {
    document.getElementById('personal_code_paragraph').innerHTML = socketId;
}
...
export const showIncomingCallDialog = (username , callType, acceptCallHandler, rejectCallHandler) => {
    const callTypeInfoText = callType === constants.callType.CHAT_PERSONAL_CODE ? 'Chat' : 'Video';
    const incomingCallDialog = elements.getIncomingCallDialog(username , callTypeInfoText, acceptCallHandler, rejectCallHandler);
...
export const showCallingDialog = (calleePersonalCode , rejectCallHandler) => {
    const callingDialog = elements.getCallingDialog(calleePersonalCode , rejectCallHandler);
    const dialog = document.getElementById('dialog');
    dialog.querySelectorAll('*').forEach(n => n.remove())
    dialog.appendChild(callingDialog); 
}

export const showInfoDialog = (preOfferAnswer , usernames = 'User') => {
    let infoDialog = '';
    if (preOfferAnswer === constants.preOfferAnswer.CALLEE_NOT_FOUND) {
        infoDialog = elements.getInfoDialog(`${usernames} not found` , 'He might be offline or busy , kisi aur ko milao');
    }
    if (preOfferAnswer === constants.preOfferAnswer.CALL_UNAVAILABLE) {
        infoDialog = elements.getInfoDialog(`${usernames} is unavailable at the moment` , 'Please try again later , kisi aur ko milao');
    }
    if (preOfferAnswer === constants.preOfferAnswer.CALL_REJECTED) {
        infoDialog = elements.getInfoDialog(`${usernames} rejected you call` , 'Banda busy ho gaya : (');
    }
    const dialog = document.getElementById('dialog');
    dialog.querySelectorAll('*').forEach(n => n.remove())
